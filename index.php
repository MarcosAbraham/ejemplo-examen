<?php
session_start();

require_once'LIGA3/LIGA.php';
BD('localhost', 'root', '', 'SCD');

$where = '';

if (!empty($_SESSION['id']) && !empty($_SESSION['pass'])){
    $where = "where id='$_SESSION[id]' and pass = '$_SESSION[pass]'";
}

$usuarios = LIGA ('usuarios', $where);

if ($usuarios->numReg() == 1){
    header('sistem.php');
}
//HTML::forma($usuarios, 'Logan', 'id, -fecha, nombre, pass');

HTML::cabeceras(array('title'=>'Login para usuarios', 'description'=>'Ingreso seguro a la página web'));

//metodo get -> datos se envian por la URL post -> En el cuerpo de la peticíon.

$campos = array('Usurname' =>'<input id="Usurname" name="id" />',
                'Contraseña'=>'<input type="password" id="Contraseña" name="pass" />');

$props = array ('form'=>'action="login.php" method="POST"');

if (isset($_GET['error'])) {
    echo '<p>Error en Datos';
}

HTML::forma($usuarios, 'Login', $campos, $props);

$js=array('js'=>array('js/jquery-3.3.1.min.jcs.js', 'js/codigo.js'));

HTML::pie($js);

?>